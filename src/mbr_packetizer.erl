...
init(Options) ->
  ...
  FFmpeg = ffmpeg:start_worker(filename:join(code:lib_dir(ffmpeg,priv), "flussonic_ffmpeg")),

  {ok, MBR} = update_options(Options, #mbr{name = Name, ffmpeg = FFmpeg}),
  {ok, MBR}.
